...
def test_incorrect_tree_structure_creation():
    ...
    with pytest.raises(TreeStructureError) as err:
        t = Tree()
        t[5] = LeafNode(index=5, value=33.3, idx_data_points=np.array([1, 2, 3], dtype='int64'))
    ...
    with pytest.raises(TreeStructureError) as err:
        t = Tree()
        t[0] = LeafNode(index=0, value=33.3, idx_data_points=np.array([1, 2, 3], dtype='int64'))
        t[1] = LeafNode(index=1, value=22.2, idx_data_points=np.array([1], dtype='int64'))
    assert str(err.value) == 'Parent node must be of class SplitNode'

    with pytest.raises(TreeStructureError) as err:
        t = Tree()
        t[0] = LeafNode(index=2, value=33.3, idx_data_points=np.array([1, 2, 3], dtype='int64'))
...
def test_correct_get_node():
    ...
    assert t[4] == LeafNode(index=4, value=22.2, idx_data_points=np.array([2], dtype='int64'))
...
def test_correct_grow_tree():
    ...
    manual_tree = {
        0: LeafNode(index=0, value=33.3, idx_data_points=np.array([1, 2, 3, 4], dtype='int64')),
    }
    assert t.tree_structure == manual_tree
    ...
    manual_tree = {
        ...
        3: LeafNode(index=3, value=11.1, idx_data_points=np.array([1], dtype='int64')),
        4: LeafNode(index=4, value=22.2, idx_data_points=np.array([2], dtype='int64')),
        5: LeafNode(index=5, value=33.3, idx_data_points=np.array([3], dtype='int64')),
        6: LeafNode(index=6, value=33.3, idx_data_points=np.array([4], dtype='int64')),
    }
    assert t.tree_structure == manual_tree
...
def test_incorrect_grow_tree():
    ...
    with pytest.raises(TreeStructureError) as err:
        new_not_split_node = LeafNode(index=2, value=666.6, idx_data_points=np.array([4, 5], dtype='int64'))
        new_left_node = LeafNode(index=3, value=666.6, idx_data_points=np.array([4], dtype='int64'))
        new_right_node = LeafNode(index=4, value=122.12, idx_data_points=np.array([5], dtype='int64'))
...
def test_incorrect_prune_tree():
    ...
    with pytest.raises(TreeStructureError) as err:
        new_leaf_node = LeafNode(index=2, value=666.6, idx_data_points=np.array([4], dtype='int64'))
        t.prune_tree(2, new_leaf_node)
    assert str(err.value) == 'Only SplitNodes are prunable'

    with pytest.raises(TreeStructureError) as err:
        new_leaf_node = LeafNode(index=0, value=666.6, idx_data_points=np.array([1, 2, 3, 4], dtype='int64'))
...
def test_correct_trees_eq():
    ...
    t2[5] = LeafNode(index=5, value=33.3, idx_data_points=np.array([3], dtype='int64'))
    t2[6] = LeafNode(index=6, value=33.3, idx_data_points=np.array([4], dtype='int64'))
...
def test_correct_get_current_idx_data_points():
    ...
    assert np.array_equal(t.get_current_idx_data_points(index_node=0), np.array([1, 2, 3], dtype='int64'))
    assert np.array_equal(t.get_current_idx_data_points(index_node=1), np.array([1, 2], dtype='int64'))
    assert np.array_equal(t.get_current_idx_data_points(index_node=2), np.array([3], dtype='int64'))
    assert np.array_equal(t.get_current_idx_data_points(index_node=3), np.array([1], dtype='int64'))
    assert np.array_equal(t.get_current_idx_data_points(index_node=4), np.array([2], dtype='int64'))
