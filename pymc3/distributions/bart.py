...
class BaseBART(NoDistribution):
    def __init__(self, X, Y, m=200, alpha=0.95, beta=2.0, cache_size=5000, *args, **kwargs):
        ...
        self.X = X
        self.Y = Y.eval()
        super().__init__(shape=X.shape[0], dtype='float64', testval=0, *args, **kwargs)  # FIXME dtype and testvalue are nonsensical
...
class BART(BaseBART):
    ...
    def draw_leaf_value(self, tree, idx_data_points):
        ...
        data_mean = node_responses.mean() / self.m # for skewed distribution use median or sample from data-points?
