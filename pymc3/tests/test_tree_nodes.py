...
def test_good_split_node_creation():
    ...
    assert np.array_equal(s_quant.idx_data_points, np.array([], dtype=int))
...
def test_good_leaf_node_creation():
    leaf_node = LeafNode(index=0, value=22.2, idx_data_points=None)
    assert leaf_node.index == 0
    assert np.array_equal(leaf_node.idx_data_points, np.array([], dtype=int))
...
def test_correct_nodes_eq():
    ...
    leaf_node2 = LeafNode(index=1, value=22.2, idx_data_points=np.array([], dtype=int))
    leaf_node3 = LeafNode(index=2, value=22.2)
    leaf_node4 = LeafNode(index=1, value=55.5)
    leaf_node5 = LeafNode(index=1, value=55.5, idx_data_points=np.array([1,2,3]))
